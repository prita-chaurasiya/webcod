    ...
    <section className="py-20 bg-transparent relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 lg:px-8 max-w-7xl relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-14">
          ...
          {/* Category Tabs */}
          <div className="flex flex-wrap items-center gap-2 p-1.5 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 shadow-sm w-full md:w-auto">
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`relative px-5 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 ${
                    active ? "text-white" : "text-slate-400 hover:text-white hover:bg-white/10"
                  }`}
                >
                  {active && (
                    <motion.div
                      layoutId="activeBlogTab"
                      className="absolute inset-0 bg-cyan-500 rounded-xl shadow-md"
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl text-sm font-medium text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all shadow-sm"
            />
          </div>
          ...
          <AnimatePresence>
            {filteredBlogs.map((blog, index) => (
              <motion.div
                ...
                className="h-full"
              >
                <div className="group bg-white/5 backdrop-blur-sm rounded-3xl overflow-hidden shadow-sm hover:shadow-[0_15px_30px_rgba(6,182,212,0.15)] hover:-translate-y-2 transition-all duration-400 border border-white/10 flex flex-col h-full relative">
                  ...
                  <div className="relative w-full h-52 overflow-hidden bg-slate-800">
                    ...
                    <div className="absolute top-4 left-4 z-10">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#0a0f1c]/80 backdrop-blur-md text-white text-[11px] font-bold tracking-wide border border-white/10">
                  ...
                  <div className="p-7 flex flex-col flex-grow">
                    ...
                    <h3 className="text-lg font-bold text-white leading-snug mb-3 group-hover:text-cyan-400 transition-colors duration-300">
                      {blog.title}
                    </h3>
                    ...
                    <div className="mt-auto pt-4 border-t border-white/10 flex items-center justify-between">
                      ...
                      <span className="w-2 h-2 rounded-full bg-cyan-500/40 group-hover:bg-cyan-400 group-hover:scale-150 transition-all duration-300 shadow-[0_0_8px_rgba(6,182,212,0.8)] opacity-0 group-hover:opacity-100"></span>
                    </div>
                  </div>
        ...
        {filteredBlogs.length === 0 && (
          <motion.div
            ...
            className="text-center py-20 bg-white/5 backdrop-blur-sm rounded-3xl border border-white/10 mt-4"
          >
            <Search className="w-12 h-12 text-slate-500 mx-auto mb-4" />
            <h4 className="text-xl font-bold text-white mb-2">No matching articles found</h4>
            <p className="text-slate-400 text-sm mb-6">Try searching with different keywords or switch categories.</p>
            <button
              onClick={() => { setSelectedCategory("All"); setSearchQuery(""); }}
              className="px-6 py-2.5 rounded-xl bg-cyan-500 text-white text-xs font-bold hover:bg-cyan-400 transition-colors shadow-[0_0_15px_rgba(6,182,212,0.3)]"
            >
              Reset Filters
            </button>
          </motion.div>
